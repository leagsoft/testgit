...
<%
TopParent=request.QueryString("Parent")
if topParent="" then
TopParent=0
end if
...
cSql = "select * from Products  where type1 in (select dicID from SYSDIC where parent='" & TopParent & "') "&cSql1&cSql4&cSql2&cSql3&cSql5
end if
...
Dim Sql
    Sql="SELECT * From ClassList Where parent in (select ID from classList where title='宣传园地') Order By OrderNum DESC,upTime DESC"
  '  Rs.PageSize=20
...
<tr>
<td width="90%" align="left" class="BigFont"><img src="Images/item.gif" border="0" width="18" height="18" align="absmiddle">
<a href="index.asp?QXMC=宣传园地&Purview=管理者">所有类别</a>
<%
do while not RsClass.eof
%>
| <a href="index.asp?QXMC=宣传园地&Purview=管理者&Parent=<%=RsClass("Id")%>"><%=RsClass("title")%></a> 
<% 
RsClass.movenext
loop
%>
...
TypeSql = "select DICID,Type from SYSDIC where DELETED=0 and parent='" & TopParent & "' order by DICID asc"
end if
...
<table width="80%" border="1" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFDFDF" bordercolorlight="#333333" bordercolordark="#FFFFFF">
  <tr>
    <td colspan="4" bgcolor="#FFDA99" valign="middle" height="30">&nbsp;<a href="PrdctList.asp">宣传园地管理</a> -> <font color=red>信息列表</font>&nbsp;&nbsp;<font color=red><%=cMsg%></font></td>
  </tr>
  <tr>
      <td bgcolor="#00DDDD" height="22" align="center">编号</td>
      <td bgcolor="#00DDDD" height="22" align="center">标题</td>
