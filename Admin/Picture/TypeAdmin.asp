...
IF Action="增加" then
	...
	cParentID=request("ParentClass")

	if cIspicture="" then
		cIsPicture=0
	end if
	Sql="insert into SYSDIC (Type,IsPicture,Parent) values ('"&cValue&"','"&cIsPicture&"','"&cParentID&"')"
...
IF Action="修改" then
	...
	If cIspicture="" then
		cIsPicture=0
	End If
	Sql="Update SYSDIC set Type='"&cValue&"',IsPicture='"&cIsPicture&"',Parent='"&cParentID&"' where DicId="&cDicId
	'Response.Write sql
	'Response.End
...
Dim Sql
    Sql="SELECT * From ClassList Where parent in (select ID from classList where title='宣传园地') Order By OrderNum  ,upTime DESC"
  '  Rs.PageSize=20
...
%>
<tr>
<td bgcolor="#ccffcc" height="20" width="20%" align="right">上一级类：</td>
<td bgcolor="#ffffff" height="20">
<select name="ParentClass">
<%
do while not rsClass.eof

	if cParentID=rsClass("ID") then
 

	%>
		
		<option value='<%=rsClass("ID")%>' selected><%=rsClass("title")%></option>
	<%
	else
	%>
	<option value='<%=rsClass("ID")%>' ><%=rsClass("title")%></option>
	<%
	end if
	rsClass.movenext
loop
