...
Sub SaveMdy()
    ...
    Dim Id,ClassId,Title,Url,Author,From,KeyWord,Filename
    ...
    Title=Request("Title")
   ' Url=Request("Url")
    FileName = Trim(Request("Url"))	'上传文档的名称
    Author=Request("Author")
    If Author="" then
		Author="0"
    end if
    '****** Add By BennyLiu:20040625 *******
    '**当是金融统计信息模块时，有此下面两个变量
		Browser=Request("Browser")			'取得可浏览者
		DocumentType=Request("DocumentType")	'取得文件类型
    '****** End Add **********
    ...
    '完成
    Response.Redirect("News_List.asp?Work="&Request.Cookies("ZGW_NewsSys3")("News_List_WorkType"))
End Sub
...
Sub AddReco()
	...
	Author=Trim(Request("Author"))		'资源作者，局长的序号
	Browser=Trim(Request("Browser"))			'Add By BennyLiu:20040625 增加浏览者
	DocumentType=Trim(Request("DocumentType"))	'Add By BennyLiu:20040706 增加文件类型
	Filename=Trim(Request("Url"))		'附件名称
    	NowTime=Now
    ...
    End If    
    Response.Write("<script>if(confirm(""<添加成功>\n是否想继续添加资源？"")){window.location='News_Add.asp?Work=AddReco'}else{window.location='"&"News_List.asp?Work="&Request.Cookies("ZGW_NewsSys3")("News_List_WorkType")&"'}</script>")
	If CBool(Request("CurrentClassIdUsed")) Then
		Response.Cookies("ZGW_NewsSys3")("CurrentClassIdUsed")=ClassId
	Else
		Response.Cookies("ZGW_NewsSys3")("CurrentClassIdUsed")=""
	End If
End Sub
...
Sub DelReco()
    ...
    Response.Write("<script>alert(""<系统提示>\n此次操作预计操作（"&UBound(IdList)+1&"）条资料,实际成功操作（"&FinishedNum&"）条，未完成（"&(UBound(IdList)+1-FinishedNum)&"）条"");window.location="""&"News_List.asp?Work="&Request.Cookies("ZGW_NewsSys3")("News_List_WorkType")&"&CurrentPage="&Request.Cookies("ZGW_NewsSys3")("News_List_CurrentPage")&""";</script>")
    Response.End
End Sub
