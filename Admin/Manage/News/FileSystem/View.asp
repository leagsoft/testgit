...
<%
dim classid,title,Author
Classid=Request("radioBoxItem")
Title=Request("Title")
Author=Trim(Request("Author"))		'资源作者		'Add By BennyLiu:20040614
if Author="" then Author="0" end if
'*******Add By BennyLiu:20040625*********
Browser=Request("Browser")			'浏览者
DocumentType=Trim(Request("DocumentType"))	'文件类型
If Browser<>"" then
	Session("Browser")=Browser			'用Session来传给下面的页面。
	Session.Timeout =21
End If
...
function UploadFile()
{
   ...
   // }
//*******Modify by BennyLiu:20040618********   
   //window.location .href("UpFile.asp?Title=<%=Title%>&Classid=<%=Classid%>&Path=<%=tempCurrentPath%>")
   window.location .href("UpFile.asp?Author=<%=Author%>&Title=<%=Title%>&Classid=<%=Classid%>&Path=<%=tempCurrentPath%>")
//******* End Modify ***********************
}
function DelItem()
{
