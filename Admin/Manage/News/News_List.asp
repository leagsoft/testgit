...
Dim Work
    Work=Request("Work")
Response.Cookies("ZGW_NewsSys3")("News_List_WorkType")=Work
Dim sType
...
Dim CurrentPage
    ...
    End If
    Response.Cookies("ZGW_NewsSys3")("News_List_CurrentPage")=CurrentPage
...
<!--        <tr>
        ...
        <tr> 
          <td> <p>　　标志某资源在被最后修改的同时是否也生成了相对应的静态文件,若生成了,则“是”，反之“<font color="#00CC00">否</font>”</p></td>
        </tr>-->
      </table>
      <a name="Help"></a></td>
  </tr>
...
Function ExeSql(user)
    ...
    Select Case Work
        ...
        Case Else            '资源列表
           ' tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Content is null And ClassTitle='"&QXMC&"' Order By Id DESC"
			'为了控制每个用户只能看到自己有权看的信息，作如下操作
			If user="管理员" then '是系统管理员
				'20050630 tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Content is null Order By Id DESC"
				tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Filename is not null And Filename<>'' Order By Id DESC"
			Else '不是系统管理员
				Strsql="select ID from classlist where title='"&QXMC&"'"
				...
				IF Rs2.EOF then			'若没有下一级，如下
					if purview ="管理者" then
						'20050630 tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Content is null And ClassTitle='"&QXMC&"' Order By Id DESC"
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Filename is not null And Filename<>'' And ClassTitle='"&QXMC&"' Order By Id DESC"
					else
						'20050630 tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Editor='"&UserName&"' And Content is null And ClassTitle='"&QXMC&"' Order By Id DESC"
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Editor='"&UserName&"' And Filename is not null And Filename<>'' And ClassTitle='"&QXMC&"' Order By Id DESC"
					end if
				Else					'若有下一级，如下
					IF parent="0" then			
						Parent=TypeID						
					...
					Elseif (purview="编辑者" and column="") then
						'20050630 tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Editor='"&UserName&"' And Content is null Order By Id DESC"
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Editor='"&UserName&"' And Filename is not null And Filename<>'' Order By Id DESC"
					Elseif(purview="编辑者" and column<>"") then						
						'20050630 tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Editor='"&UserName&"' And Content is null Order By Id DESC"
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Editor='"&UserName&"' And Filename is not null And Filename<>'' Order By Id DESC"						
					End If
