...
Else
			...
			Rs1.Close
			set Rs1=nothing
			Sql="select * from classlist where Parent="&QXID
...
Function ExeSql(user)
    ...
    Select Case Work
        Case "Dustbin"        '回收站列表
            'tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &")  And Del=1 And Content is null And ClassTitle='"&QXMC&"' Order By Id DESC"
        ...
        Case "UnChecked"    '未审核列表
            tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &")  And IsChecked=0 And Del=0 And Content is null Order By Id DESC"
        Case Else            '资源列表
