...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	//	  Call CreateClassTree1(SysAdmin.AdminTopClassId,CLng(CurrentClassIdUsed))
	  ...
	  //Call CreateClassTree1("457","","")
	//  Call CreateClassTree1(QXMC,CLng(CurrentClassIdUsed))
...
%>      
    ...
    <script language="javascript">
	  ...
	  End If
		...
		// Call CreateClassTree3(411,CLng(CurrentClassIdUsed))
...
'**    Dim radioSelected
    While Not Rs.Eof
        ...
        End If
		'CreateClassTree1 CuClassTitle,Column,CuClassId
...
'生成栏目树
Sub CreateClassTree5(ParentName,ClassId)
    Dim Sql
        Sql="Select Id,Parent,Title From ClassList Where Parent in ( select ID from classlist where title='"&ParentName &"')"
    Dim Rs
    Set Rs=Conn.ExeCute(Sql)
    Dim radioSelected
    While Not Rs.Eof
        ...
        Else
            Response.Write "root1.CreateNode("&Rs("Id")&","&Rs("Parent")&",""<INPUT "&radioSelected&" TYPE=\""radio\"" NAME=\""radioBoxItem\"" value=\"""&Rs("Id")&"\"">"&Rs("Title")&""")" & vbCrLf
        End If
        CreateClassTree5 Rs("Id"),ClassId
        Rs.MoveNext
    Wend
    Rs.Close
    Set Rs=Nothing
