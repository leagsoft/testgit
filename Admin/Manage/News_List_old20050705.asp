...
Function ExeSql(user)
    ...
    Select Case Work
        ...
        Case Else            '资源列表
           ' tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Content is Not null And ClassTitle='"&QXMC&"' Order By Id DESC"
			'为了控制每个用户只能看到自己有权看的信息，作如下操作
			If user="管理员" then 
				tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Content is Not null Order By Id DESC"
			Else 
				Strsql="select ID from classlist where title='"&QXMC&"'"
...
'**					tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Content is Not null And ClassTitle='"&QXMC&"' Order By Id DESC"
					if purview ="管理者" then
						...
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent) &") And Del=0 And Editor='"&UserName&"' And Content is Not null And ClassTitle='"&QXMC&"' Order By Id DESC"
					end if
				Else					'若有下一级，如下
					IF parent="0" then			
						Parent=TypeID						
...
'**					tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0  And Content is Not null Order By Id DESC"
					...
					If (purview = "管理者" and column="") then
						'***tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And ClassTitle='"&Column&"' And Content is Not null Order By Id DESC"
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Content is Not null Order By Id DESC"										
					Elseif(purview="管理者" and column<>"") then
						'Parent=GetClassID(QXMC)						
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Content is Not null Order By Id DESC"
					Elseif (purview="编辑者" and column="") then
						'***tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And ClassTitle='"&Column&"' And Editor='"&UserName&"' And Content is Not null Order By Id DESC"
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Editor='"&UserName&"' And Content is Not null Order By Id DESC"										
					Elseif(purview="编辑者" and column<>"") then
						'Parent=GetClassID(QXMC)
						tSql="SELECT Id,Class,Title,EditorTitle,AddTime,UpTime,ClassTitle,Del,Created,IsChecked,FilePath From view_AllNewsInfo Where "&sType&" Like '%"&sKey&"%' And Class In ("& Parent&AllChildClass(Parent)&") And Del=0 And Editor='"&UserName&"' And Content is Not null Order By Id DESC"						
					End If
